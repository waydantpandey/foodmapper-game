...
async function createFolderStructureInSharedDrive() {
  try {
    console.log('🔍 Looking for your shared "food data 1" folder...');
    ...
    sharedFolders.data.files.forEach(file => {
      const parentInfo = file.parents ? ` (parent: ${file.parents[0]})` : ' (root)';
      console.log(`  - ${file.name}${parentInfo}`);
    });

    // Look for "food data 1" folder
    const foodDataFolder = sharedFolders.data.files.find(f => f.name === 'food data 1');
    
    if (!foodDataFolder) {
      console.log('❌ "food data 1" folder not found in accessible folders');
      console.log('💡 Make sure you\'ve shared the folder with: food-scraper@foodmapper-472618.iam.gserviceaccount.com');
      return;
    }
