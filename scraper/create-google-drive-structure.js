...
if (!fs.existsSync(credentialsPath)) {
  console.error('❌ credentials.json not found. Please place your Google Service Account credentials in the scraper folder.');
  process.exit(1);
}
...
function sanitizeFolderName(name) {
  // Replace problematic characters for Google Drive folder names
...
async function findOrCreateFolder(name, parentId = null) {
  try {
    ...
    console.error(`❌ Error with folder ${sanitizedName}:`, error.message);
    throw error;
  }
}
...
async function createFolderStructure() {
  try {
    console.log('🚀 Starting Google Drive folder structure creation...');
    
    // Find or create the main "food data 1" folder
    const mainFolderId = await findOrCreateFolder('food data 1');
    ...
    for (const countryData of foodData) {
      ...
      const countryFolderId = await findOrCreateFolder(countryData.country, mainFolderId);
      totalFolders++;
    ...
    console.log(`\n📁 Main folder: "food data 1"`);
    console.log(`🌍 Each country has its own folder with dish subfolders`);
